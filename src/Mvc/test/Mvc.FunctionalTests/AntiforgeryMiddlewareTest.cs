...
public class AntiforgeryMiddlewareTest
{
    ...
    public async Task Works_WithAntiforgeryMetadata_ValidToken()
    {
        ...
        UseSpecificControllers(builder.Services.AddMvcCore(), typeof(TestController));
    ...
    {
        var builder = WebApplication.CreateBuilder();
        UseSpecificControllers(builder.Services.AddMvcCore(), typeof(TestWithRequestSizeLimitController));
        builder.Services.AddControllersWithViews();
        builder.Services.AddAntiforgery();
        builder.WebHost.UseTestServer();
        await using var app = builder.Build();
    ...
    internal void UseSpecificControllers(
    ...
    internal IMvcCoreBuilder UseSpecificControllers(
        IMvcCoreBuilder mvcCoreBuilder,
        params Type[] controllerTypes) => mvcCoreBuilder
        .ConfigureApplicationPartManager(partManager => UseSpecificControllers(partManager, controllerTypes));

    private class SelectedControllersApplicationParts(Type[] types) : ApplicationPart, IApplicationPartTypeProvider
    {
